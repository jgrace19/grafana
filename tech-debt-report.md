# Tech Debt Report — all — 2026-06-02

## Hotspots (high debt × high churn)
| Rank | Area | Signals | Commits (6 months) | Priority Score |
|------|------|---------|-------------------|----------------|
| 1 | `pkg/registry/` | 327.0 | 504 | 2936.51 |
| 2 | `pkg/tests/` | 322.0 | 460 | 2849.26 |
| 3 | `public/app/plugins/datasource/` | 285.0 | 141 | 2037.68 |
| 4 | `pkg/storage/` | 240.0 | 308 | 1985.15 |
| 5 | `pkg/services/ngalert/` | 211.0 | 151 | 1529.31 |
| 6 | `public/app/features/dashboard/` | 151.0 | 128 | 1058.7 |
| 7 | `public/app/features/alerting/` | 132.0 | 212 | 1020.98 |
| 8 | `public/app/plugins/panel/` | 115.0 | 140 | 821.05 |
| 9 | `pkg/api/` | 113.0 | 114 | 773.54 |
| 10 | `pkg/services/libraryelements/` | 122.0 | 12 | 451.45 |
...
## Recommended Actions
1. **[Tech Debt] Migrate dashboard class/connect components to hooks** — Focus `public/app/features/dashboard/` legacy React patterns. Use the `migrate-class-components` skill for class/connect migrations.
2. **[Tech Debt] Modernize Explore TraceView (stylesFactory + unsafe lifecycle)** — Remove the remaining unsafe lifecycle and replace `stylesFactory` in TraceView.
3. **[Tech Debt] Split oversized Go files (setting.go, dashboard_service.go, storage_backend.go)** — Prioritize the largest high-churn service files.
4. **[Tech Debt] Clean up deprecated feature toggles and migrate IsEnabled API** — Replace old feature flag checks in `pkg/` and retire deprecated registry entries.
5. **[Tech Debt] Reduce explicit any in top 10 frontend files** — Start with the top 10 files listed above for high-signal type-safety gains.
6. **[Tech Debt] Reduce debt concentration in pkg/registry, pkg/storage, and pkg/services/ngalert** — Tackle backend TODO/nolint/oversized hotspots where churn stays high.

## Change Log

### 2026-06-02 (current scan)
...
*Note*: File-level resolved/new tracking is partially limited because prior reports stored aggregate metrics but not complete per-category file inventories.
