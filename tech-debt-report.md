# Tech Debt Report - all - 2026-06-08
...
| 9 | `public/app/plugins/panel/` | 115 | 138 | 818.68 |
| 10 | `pkg/api/` | 113 | 109 | 766.29 |
...
| Toggle Name | Active Go Call Sites (outside registry) | Sample Paths |
|---|---:|---|
| `localeFormatPreference` | 0 | none |
...
| `prometheusTypeMigration` | 0 | none |

### Old `IsEnabled`/`IsEnabledGlobally` API: 161 files

## Recommended Actions

1. **Dashboard modernization sweep** - Migrate class components and `connect()` usage in `public/app/features/dashboard/` with the `migrate-class-components` skill.
2. **Plugin datasource modernization** - Prioritize `public/app/plugins/datasource/` where legacy React patterns and high TODO density overlap with active churn.
3. **Explore TraceView modernization** - Remove the remaining unsafe lifecycle and `stylesFactory` usage in the TraceView subtree.
4. **Decompose oversized Go modules** - Split the top oversized files (`pkg/services/featuremgmt/registry.go`, `pkg/setting/setting.go`, `pkg/services/dashboards/service/dashboard_service.go`) into focused packages.
5. **OpenFeature migration** - Replace remaining `IsEnabled`/`IsEnabledGlobally` call sites; see `pkg/services/featuremgmt/` migration docs.
6. **Reduce explicit any in top offenders** - Start with the top 10 files listed in Type Safety.
...
**New since last scan:**
- None detected from aggregate metrics

_Note: previous report stored partial file-level lists; resolved/new details are tracked here at aggregate-metric level._
