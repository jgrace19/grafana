...
import { unifiedAlertListStyles } from './UnifiedAlertList.stylex';
...
function UnifiedAlertList(props: PanelProps<UnifiedAlertListOptions>) {
  ...
  return (
    <ScrollContainer minHeight="100%">
      {havePreviousResults && noAlertsMessage && <div {...stylex.props(unifiedAlertListStyles.noAlertsMessage)}>{noAlertsMessage}</div>}
      {havePreviousResults && (
        <section>
...
function StatView({
  ...
  if (enhancementsEnabled) {
    ...
    return (
      <Link href={href} {...stylex.props(unifiedAlertListStyles.statLink)}>
        {bigValue}
      </Link>
    );
  }
