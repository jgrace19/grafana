...
const livePanelDebugLog = createDebugLog('live-panel', 'LivePanel');
...
export class LivePanel extends PureComponent<Props, State> {
  ...
  streamObserver: PartialObserver<LiveChannelEvent> = {
    next: (event: LiveChannelEvent) => {
      if (isLiveChannelStatusEvent(event)) {
        ...
        livePanelDebugLog('ignore', event);
      }
    },
  };
  ...
  async loadChannel() {
    ...
    if (!isValidLiveChannelAddress(addr)) {
      livePanelDebugLog('INVALID', addr);
      this.unsubscribe();
      this.setState({
        addr: undefined,
      });
    ...
    if (isEqual(addr, this.state.addr)) {
      livePanelDebugLog('Same channel', this.state.addr);
      return;
    }
