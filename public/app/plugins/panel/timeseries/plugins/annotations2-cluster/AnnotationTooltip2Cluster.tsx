...
import { annotationTooltip2ClusterStyles } from './AnnotationTooltip2Cluster.stylex';
...
export const AnnotationTooltip2Cluster = ({
  ...
  for (let i = 0; i < annoVals.time.length; i++) {
    if (annoVals.clusterIdx?.[i] === clusterIdx && i !== annoIdx) {
      ...
      annotationTooltipComponents.push(
        <div className={annotationCount % 2 !== 0 ? (annotationTooltip2ClusterStyles.zebra) : styles.annotationWrapper}>
  ...
  return (
    <div data-testid={selectors.pages.Dashboard.Annotations.clusterTooltip} {...stylex.props(annotationTooltip2ClusterStyles.wrapper)}>
      <ScrollContainer maxHeight="260px">
        ...
        {annotationTooltipComponents.map((item, clusterIndex) => (
          <div key={clusterIndex}>
            {item}
            <div {...stylex.props(annotationTooltip2ClusterStyles.hr)}></div>
          </div>
        ))}
