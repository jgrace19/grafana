...
export const TimeSeriesPanel = ({
  ...
  const { frames, compareDiffMs } = useMemo(() => {
    const baseFrames = prepareGraphableFields(data.series, config.theme2, timeRange);
    if (baseFrames != null) {
      // Append trend overlay frames so they participate in the same legend,
      // tooltip, and GraphNG join paths as regular series.
      const frames = appendTrendOverlayFrames(baseFrames, options.trendOverlay, config.theme2);
