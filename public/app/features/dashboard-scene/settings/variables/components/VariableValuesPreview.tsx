...
import { variableValuesPreviewStyles } from './VariableValuesPreview.stylex';
...
export const VariableValuesPreview = ({ options, staticOptions }: VariableValuesPreviewProps) => {
  ...
  return (
    <div {...stylex.props(variableValuesPreviewStyles.previewContainer)} style={{ gap: '8px' }}>
...
function VariableValuesWithPropsPreview({
  ...
  return (
    <div data-testid={selectors.pages.Dashboard.Settings.Variables.Edit.CustomVariable.previewTable}>
      <InteractiveTable
        {...stylex.props(variableValuesPreviewStyles.table)}
        columns={columns}
        data={data}
    ...
    <>
      <InlineFieldRow>
        {previewOptions.map((o, index) => (
          <InlineFieldRow key={`${o.value}-${index}`} {...stylex.props(variableValuesPreviewStyles.optionContainer)}>
            <InlineLabel data-testid={selectors.pages.Dashboard.Settings.Variables.Edit.General.previewOfValuesOption}>
              <div {...stylex.props(variableValuesPreviewStyles.label)}>{o.label || String(o.value)}</div>
            </InlineLabel>
          </InlineFieldRow>
        ))}
      </InlineFieldRow>
      {options.length > previewLimit && (
        <InlineFieldRow {...stylex.props(variableValuesPreviewStyles.optionContainer)}>
