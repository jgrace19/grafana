...
import { logDetailsRowStyles } from './LogDetailsRow.stylex';
...
const cn = (key: keyof typeof logDetailsRowStyles) =>
  mergeStylexClassName(stylex.props(logDetailsRowStyles[key]), undefined).className ?? '';

const getStyles = memoizeOne((_theme: GrafanaTheme2) => ({
  labelType: cn('labelType'),
  wordBreakAll: cn('wordBreakAll'),
  copyButton: cn('copyButton'),
  adjoiningLinkButton: cn('adjoiningLinkButton'),
  wrapLine: cn('wrapLine'),
  logDetailsStats: cn('logDetailsStats'),
  logDetailsValue: cn('logDetailsValue'),
  buttonRow: cn('buttonRow'),
}));
    ...
    return (
      <>
        <tr className={rowStyles.logDetailsValue}>
          ...
          <td className={clsx(styles.wordBreakAll, wrapLogMessage && styles.wrapLine)}>
            <div className={styles.logDetailsValue}>
              ...
              <div className={clsx((singleVal || isMultiParsedValueWithNoContent) && styles.adjoiningLinkButton)}>
                {links?.map((link, i) => {
