...
export const DashboardPicker = ({ dashboardUid, panelId, isOpen, onChange, onDismiss }: DashboardPickerProps) => {
  ...
  const DashboardRow = ({ index, style }: { index: number; style?: CSSProperties }) => {
    ...
    return (
      <button
        ...
        style={style}
        {...mergeStylexClassName(stylex.props(dashboardPickerStyles.rowButton), { [stylex.props(dashboardPickerStyles.rowOdd)]: index % 2 === 1, [stylex.props(dashboardPickerStyles.rowSelected)]: isSelected })}
        onClick={() => handleDashboardChange(dashboard.uid)}
      >
        <div {...mergeStylexClassName(stylex.props(dashboardPickerStyles.dashboardTitle), stylex.props(dashboardPickerStyles.rowButtonTitle))}>{dashboard.name}</div>
        <div {...stylex.props(dashboardPickerStyles.dashboardFolder)}>
  ...
  const PanelRow = ({ index, style }: { index: number; style: CSSProperties }) => {
    ...
    return (
      <button
        ...
        style={style}
        disabled={disabled}
        {...mergeStylexClassName(stylex.props(dashboardPickerStyles.rowButton), stylex.props(dashboardPickerStyles.panelButton), {
          [stylex.props(dashboardPickerStyles.rowOdd)]: index % 2 === 1,
          [stylex.props(dashboardPickerStyles.rowSelected)]: isSelected,
        })}
        onClick={() => (disabled ? noop : setSelectedPanelId(panel.id))}
      >
        <div {...stylex.props(dashboardPickerStyles.rowButtonTitle)} title={panelTitle}>
          {panelTitle}
        </div>
        {!isAlertingCompatible && !disabled && (
          ...
          >
            <Icon name="exclamation-triangle" {...stylex.props(dashboardPickerStyles.warnIcon)} data-testid="warning-icon" />
          </Tooltip>
        )}
        {disabled && (
          <Tooltip
  ...
  return (
    <Modal
      ...
      isOpen={isOpen}
      onDismiss={onDismiss}
      {...stylex.props(dashboardPickerStyles.modal)}
      contentClassName={stylex.props(dashboardPickerStyles.modalContent)}
    >
      ...
      {!selectedDashboardIsInPageResult && dashboardUid && dashboard && (
        <Alert
          ...
          topSpacing={0}
          bottomSpacing={1}
          {...stylex.props(dashboardPickerStyles.modalAlert)}
        >
          <div>
            <Trans
      ...
      <div {...stylex.props(dashboardPickerStyles.container)}>
        <FilterInput
        ...
        <div {...stylex.props(dashboardPickerStyles.column)}>
          {isDashSearchFetching && (
            <LoadingPlaceholder
              text={t('alerting.dashboard-picker.text-loading-dashboards', 'Loading dashboards...')}
              {...stylex.props(dashboardPickerStyles.loadingPlaceholder)}
            />
          )}
        ...
        <div {...stylex.props(dashboardPickerStyles.column)}>
          {!selectedDashboardUid && !isDashboardFetching && (
            <div {...stylex.props(dashboardPickerStyles.selectDashboardPlaceholder)}>
              <div>
