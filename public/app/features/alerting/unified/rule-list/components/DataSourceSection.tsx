...
import { dataSourceSectionStyles } from './DataSourceSection.stylex';
...
export const DataSourceSection = ({
  ...
  const styles = ((theme) => getStyles(theme, isCollapsed));
    ...
    <section aria-labelledby={`datasource-${String(uid)}-heading`} role="listitem">
      <Stack direction="column" gap={0}>
        <Stack direction="column" gap={0}>
          ...
          <div {...stylex.props(dataSourceSectionStyles.dataSourceSectionTitle)}>
            {loader ?? (
        ...
        {!isCollapsed && <div {...stylex.props(dataSourceSectionStyles.itemsWrapper)}>{children}</div>}
      </Stack>
    </section>
  );
};
