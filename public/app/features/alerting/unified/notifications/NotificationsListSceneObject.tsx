...
import { type TimeRange, dateTimeFormat } from '@grafana/data';
...
import './NotificationsListSceneObject.css';
...
function ListHeader() {
  return (
    <div {...stylex.props(styles.mainHeader)}>
      ...
      <div {...stylex.props(styles.stateCol)}>
        <Text variant="body">
      ...
      <div {...stylex.props(styles.statusCol)}>{/* Status badge column */}</div>
      <div {...stylex.props(styles.receiverCol)}>
        <Text variant="body">
      ...
      {config.featureToggles.alertingNotificationHistoryDetail && (
        <div {...stylex.props(styles.viewCol)}>{/* View link column */}</div>
      )}
    </div>
  );
}
...
function NotificationRow({ record, onLabelClick }: NotificationRowProps) {
  ...
  return (
    <Stack direction="column" gap={0}>
      <div
        {...stylex.props(styles.header, isCollapsed ? styles.collapsedHeader : styles.notCollapsedHeader)}
        data-testid="notification-row-header"
      >
        <CollapseToggle
          size="sm"
          className="gf-notification-row-toggle"
        ...
        {config.featureToggles.alertingNotificationHistoryDetail && (
          <div {...stylex.props(styles.viewCol)}>
            <LinkButton
...
function NotificationDetails({ record }: NotificationDetailsProps) {
  ...
  const renderAlert = (alert: CreateNotificationsqueryalertsNotificationEntryAlert, index: number) => {
    ...
    return (
      <div key={index} {...stylex.props(styles.alertDetail)}>
...
// CollapseToggle's Button overrides live in NotificationsListSceneObject.css.
