...
import { ellipseStyles } from './ellipse.stylex';
...
const Ellipse = (props: CanvasElementProps<CanvasElementConfig, CanvasElementData>) => {
  ...
  return (
    <div {...stylex.props(ellipseStyles.container)}>
      <svg
      ...
      >
        ...
        <ellipse
          ...
          ry="50%"
          clipPath={`url(#ellipseClip-${uniqueId})`}
          {...stylex.props(ellipseStyles.elementBorder)}
        />
      </svg>

      <span {...stylex.props(ellipseStyles.text)}>{data?.text}</span>
    </div>
  );
};
