...
import { annotationResultMapperStyles } from './AnnotationResultMapper.stylex';
...
export const AnnotationFieldMapper = memo(({ response, mappings, change }: Props) => {
  ...
  const renderRow = useCallback(
    (row: AnnotationFieldInfo, mapping: AnnotationEventFieldMapping, first?: AnnotationEvent) => {
      ...
      return (
        <tr key={row.key}>
          ...
          <td {...stylex.props(annotationResultMapperStyles.valueCell)}>
            {value ? (
    ...
    [fieldNames, onFieldNameChange, annotationResultMapperStyles.valueCell]
  );
