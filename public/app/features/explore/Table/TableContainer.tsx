...
export function TableContainer({
  ...
  const loadingInState = useSelector(selectIsWaitingForData(exploreId));
  const loading = tableResult && tableResult.length > 0 ? false : loadingInState;

  const hasSubFrames = (data: DataFrame) => data.fields.some((f) => f.type === FieldType.nestedFrames);

  const getTableHeight = (rowCount: number, hasSubFramesInData: boolean) => {
    if (rowCount === 0) {
      return 200;
    }
    const height = Math.min(600, Math.max(rowCount * 36, hasSubFramesInData ? 300 : 0) + 40 + 46);
    return Math.max(height, 300);
  };

  const getTableTitle = (dataFrames: DataFrame[] | null, data: DataFrame, i: number) => {
    let name = data.name;
