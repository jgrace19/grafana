...
import { breakableTextStyles } from './BreakableText.stylex';
...
export default function BreakableText({ text, className, wordRegexp = WORD_RX }: Props): React.ReactElement | null {
  ...
  while (match) {
    spans.push(
      <span key={`${text}-${spans.length}`} className={className || mergeStylexClassName(stylex.props(breakableTextStyles.BreakableText), undefined).className}>
        {match[0]}
      </span>
    );
