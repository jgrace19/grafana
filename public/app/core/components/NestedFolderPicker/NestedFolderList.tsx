...
import { nestedFolderListStyles } from './NestedFolderList.stylex';
...
export function NestedFolderList({
  ...
  return (
    <div {...stylex.props(nestedFolderListStyles.table)} role="tree">
      {items.length > 0 ? (
        <InfiniteLoader
        ...
        <div {...stylex.props(nestedFolderListStyles.emptyMessage)}>
...
function Row({ index, style: virtualStyles, data }: RowProps) {
  ...
  if (item.kind === 'ui' && item.uiKind === 'pagination-placeholder') {
    return (
      <span style={virtualStyles} {...stylex.props(nestedFolderListStyles.row)}>
  ...
  return (
    ...
    <div
      ref={rowRef}
      style={virtualStyles}
      {...mergeStylexClassName(stylex.props(nestedFolderListStyles.row, {
        [nestedFolderListStyles.rowFocused]: index === focusedItemIndex,
        [nestedFolderListStyles.rowSelected]: item.uid === selectedFolder,
      }), undefined)}
      tabIndex={-1}
    ...
    >
      <div className={nestedFolderListStyles.rowBody}>
        <Indent level={level} spacing={2} />
        ...
        {foldersAreOpenable && !emptyFolders.has(item.uid) ? (
          ...
          <span {...stylex.props(nestedFolderListStyles.folderButtonSpacer)} />
        )}

        <label {...stylex.props(nestedFolderListStyles.label)} id={labelId}>
          <Text truncate>{item.title}</Text>
          <FolderRepo folder={item} />
        </label>
        {teamOwner && (
          <div {...stylex.props(nestedFolderListStyles.teamOwner)}>
